...
def generate_mock_pest_reports(city, num_reports=15):
    ...
    try:
        # Get coordinates from our predefined dictionary
        if city in egyptian_cities:
            base_lat, base_lon = egyptian_cities[city]
        else:
            # Fallback to geocoding service with longer timeout
...
def create_pest_map(city, reports):
    ...
    try:
        ...
        # Add pest reports if we have any
        if reports:
            for report in reports:
                # Different colors based on disease
            ...
            # Add heatmap for outbreak visualization
...
if reports:
    ...
    st.dataframe(
        df_reports.sort_values('date', ascending=False).head(5),
        hide_index=True
    )
